...
public class Criptografia extends RecursiveTask<String> {
    ...
    public String palavra; //posicao do vetor de string
    //public String divisaoTexto[]= new String[2]; //
    
    public Criptografia(String palavra){ //construtor recebendo a posicao da string a ser encriptografada
        this.palavra = palavra; //atualizando a posicao
    }
    
    
    @Override
    protected String compute(){ //metodo compute
        retorno = encriptografar(palavra, chave); //chamada do metodo encriptografar
        return retorno; //retorno da encriptação
    }
    private String encriptografar(String texto, int chave){ //metodo encriptar
